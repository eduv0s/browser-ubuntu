...
const APP_VERSION = '0.9.3-webvm';
const CHEERPX_VERSION = '1.3.9';
// WebVM official Alpine image URL — this is what webvm.io/alpine.html uses
...
const BOOT_COMMAND = 'dbus-daemon --system --fork; lightdm';
...
async function boot(display, consoleElement, screenWrap, state) {
  ...
  try {
    if (!crossOriginIsolated) throw new Error('Necesita COOP/COEP. Usa Chrome/Chromium.');
    ...
    setState(state, 'Conectando imagen…');
